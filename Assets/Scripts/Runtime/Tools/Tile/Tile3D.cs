            ...
            public void Quad(Vector3 a, Vector3 b, Vector3 c, Vector3 d, Face face)
            {
                ...
                Vector2 uva, uvb, uvc, uvd;
                {
                    if (!collider)
                    {
                        ...
                        temp[0] = new Vector2((center.x + s * flipx) * uvTileSize.x,
                            (center.y - s * flipy) * uvTileSize.y);
                        temp[1] = new Vector2((center.x - s * flipx) * uvTileSize.x,
                            (center.y - s * flipy) * uvTileSize.y);
                        temp[2] = new Vector2((center.x - s * flipx) * uvTileSize.x,
                            (center.y + s * flipy) * uvTileSize.y);
                        temp[3] = new Vector2((center.x + s * flipx) * uvTileSize.x,
                            (center.y + s * flipy) * uvTileSize.y);
