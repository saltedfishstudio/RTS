...
namespace RTS.Tools.Tile.Editor
{
    [CustomEditor(typeof(Tile3D))]
    public class Tile3DEditor : UnityEditor.Editor
    {

        public Tile3D tiler
        {
            get { return (Tile3D) target; }
        }

        public Vector3 origin
        {
            get { return tiler.transform.position; }
        }
        ...
        protected virtual void OnSceneGUI()
        {
            ...
            {
                // mode toolbar
                toolMode = (ToolModes) GUI.Toolbar(new Rect(10, 10, 200, 30), (int) toolMode,
                    new[] {"Move", "Build", "Paint"});
            ...
            if (toolMode == ToolModes.Building)
            {
                ...
                if (!draggingBlock && interacting)
                {
                    ...
                    else if (e.type == EventType.MouseDrag && selected != null && hover != null &&
                             !selected.Tiles.Contains(hover.Tile))
                    {
            ...
            if ((e.type == EventType.MouseMove || e.type == EventType.MouseDrag) && interacting && !draggingBlock)
            {
                ...
                if ((hover == null && next != null) || (hover != null && next == null) ||
                    (hover != null && next != null && (hover.Tile != next.Tile || hover.Face != next.Face)))
                    invokeRepaint = true;
                hover = next;
            }

            // painting
            if (toolMode == ToolModes.Painting && (e.type == EventType.MouseDown || e.type == EventType.MouseDrag) &&
                interacting && hover != null)
            {
            ...
            if (toolMode == ToolModes.Painting && (e.type == EventType.MouseDown || e.type == EventType.MouseDrag) &&
                e.button == 1 && !e.control && !e.alt && hover != null)
            {
        ...
        {
            var e = Event.current;
            var hover = !selected && e.mousePosition.x > rect.x && e.mousePosition.y > rect.y &&
                        e.mousePosition.x < rect.xMax && e.mousePosition.y < rect.yMax;
            ...
            if (tile.HasValue)
            {
                var coords = new Rect(tile.Value.x * tiler.UVTileSize.x, tile.Value.y * tiler.UVTileSize.y,
                    tiler.UVTileSize.x, tiler.UVTileSize.y);
                GUI.DrawTextureWithTexCoords(new Rect(rect.x + 2, rect.y + 2, rect.width - 4, rect.height - 4),
                    tiler.Texture, coords);
            }
            else
            {
                EditorGUI.DrawRect(new Rect(rect.x + 2, rect.y + 2, rect.width - 4, rect.height - 4), Color.white);
                EditorGUI.DrawRect(new Rect(rect.x + 2, rect.y + 2, (rect.width - 4) / 2, (rect.height - 4) / 2),
                    Color.gray);
                EditorGUI.DrawRect(
                    new Rect(rect.x + 2 + (rect.width - 4) / 2, rect.y + 2 + (rect.height - 4) / 2,
                        (rect.width - 4) / 2, (rect.height - 4) / 2), Color.gray);
            }
